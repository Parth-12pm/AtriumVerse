...
async def get_conversation_messages(
    ...
    device_id: UUID = Query(
        ..., description="The requesting device ID to fetch specific ciphertexts for"
    ),
    ...
    for row in rows:
        ...
        if getattr(msg, "is_encrypted", False):
            if row.encrypted_ciphertext is not None:
                ...
                row.ciphertext_active_device is None
                and row.deleted_device_id is not None
            ):
...
async def send_direct_message(
    ...
    try:
        ...
        epoch_query = (
            select(DmEpoch)
            .where(DmEpoch.user_a_id == user_a_id, DmEpoch.user_b_id == user_b_id)
            .with_for_update()
        )
        ...
        if not dm_epoch:
            dm_epoch = DmEpoch(
                user_a_id=user_a_id, user_b_id=user_b_id, current_epoch=1
            )
            db.add(dm_epoch)
...
async def submit_device_keys(
    ...
    if msg.sender_id != current_user.id:
        raise HTTPException(
            status_code=403, detail="Only sender can attach device keys"
        )
...
async def get_my_dm_ciphertext(
    ...
    sender_result = await db.execute(
        select(Device).where(
            Device.id
            == message.sender_device_id,  # adjust field name to match your DM model
            Device.deleted_at == None,
        )
    )
